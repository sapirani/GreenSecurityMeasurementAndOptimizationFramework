...
class RewardCalc:
    def __init__(self, top_logtypes, dt_manager, splunk_tools, rule_frequency, num_of_searches, measurment_tool, alpha, beta, gamma, env_id):
    ...
    def get_no_agent_reward(self, time_range):
        ...
        if not relevant_row.empty:
            alert_val = relevant_row['alert_values'].values[0]
            duration_val = relevant_row['duration_values'].values[0]
        else:
            ...
            self.no_agent_values = pd.concat([self.no_agent_values, pd.DataFrame({'start_time':[time_range[0]], 'end_time':[time_range[1]], **{f"rule_alert_{saved_searches[i]}":[alert_vals[i]] for i in range(len(saved_searches))},**{f"rule_duration_{saved_searches[i]}":[duration_vals[i]] for i in range(len(saved_searches))}, 'alert_values':[alert_val], 'duration_values':[duration_val]})])
            current_minute = datetime.datetime.now().minute
            if current_minute % 5 == 0:
                self.no_agent_values.to_csv(f"/home/shouei/GreenSecurity-FirstExperiment/SplunkResearch/experiments__/{self.env_id}/no_agent_values.csv", index=False)
            relevant_row = self.no_agent_values[(self.no_agent_values['start_time'] == time_range[0]) & (self.no_agent_values['end_time'] == time_range[1])]
...
class RewardCalc1(RewardCalc):
    def __init__(self, top_logtypes, dt_manager, splunk_tools, rule_frequency, num_of_searches, measurment_tool, alpha, beta, gamma, env_id):
        super().__init__(top_logtypes, dt_manager, splunk_tools, rule_frequency, num_of_searches, measurment_tool, alpha, beta, gamma, env_id)
    ...
    def get_full_reward(self, time_range, real_distribution, current_state):
        ...
        no_agent_alert_val, no_agent_duration_val = self.no_agent_last_row['alert_values'].values[0], self.no_agent_last_row['duration_values'].values[0]
        ###### Alert Component ######
