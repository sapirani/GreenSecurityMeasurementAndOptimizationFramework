...
def save_assets(current_dir, env, mode='train'):
    ...
    with open(f'{current_dir}/{mode}/time_rules_energy_dict.json', 'w') as fp:
        json.dump(env.time_rules_energy_dict, fp)   
...
if __name__ == "__main__":
    ...
    reward_parameter_dict = {'alpha': 0.85, 'beta': 0.05, 'gamma': 0.05, 'delta': 0.05}
    ...
    savedsearches = choose_random_rules(splunk_tools_instance, num_of_searches, get_only_enabled=True)
    ...
    with open(f'{current_dir}/parameters.json', 'w') as fp:
        json.dump({'fake_start_datetime': str(fake_start_datetime), 'rule_frequency': rule_frequency, 'running_time': running_time, 'num_of_searches': num_of_searches, 'num_of_episodes': num_of_episodes, 'max_actions_value': max_actions_value, 'reward_parameter_dict': reward_parameter_dict, 'savedsearches': savedsearches}, fp)
    ...
    if mode == 'train':
        env = gym.make('splunk_attack-v0', log_generator_instance = log_generator_instance, splunk_tools_instance = splunk_tools_instance, dt_manager=dt_manager, time_range=time_range, rule_frequency=rule_frequency, reward_parameter_dict=reward_parameter_dict, max_actions_value=max_actions_value)
