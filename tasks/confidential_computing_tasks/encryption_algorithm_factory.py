...
class EncryptionAlgorithmFactory:
    ...
    def create_security_algorithm(encryption_algorithm: EncryptionType, cipher_block_mode: Optional[str],
                                  min_key_val: int = PRIME_MIN_VAL,
                                  max_key_val: int = PRIME_MAX_VAL) -> SecurityAlgorithm:

        if encryption_algorithm == EncryptionType.FernetAES:
            ...
            alg = EncryptionAlgorithmFactory.create_homomorphic_algorithm(encryption_algorithm, min_key_val,
                                                                           max_key_val)
            if alg:
                return alg
            else:
    ...
    @staticmethod
    def create_homomorphic_algorithm(encryption_algorithm: EncryptionType, min_key_val: int = PRIME_MIN_VAL,
                                     max_key_val: int = PRIME_MAX_VAL) -> HomomorphicSecurityAlgorithm:
        if encryption_algorithm == EncryptionType.Paillier:
            ...
            raise ValueError("Unknown Homomorphic encryption algorithm")
